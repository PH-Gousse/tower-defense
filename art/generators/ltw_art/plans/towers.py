...
def turret_on_base(p: dict, rng) -> Layout:
    ...
    if p["roof"] == "cone":
        ...
        lay.add(Prim("cone", (r * 0.6, 0.7), (0, top + 0.24 + 0.35, 0), slot="roof", seg=8, bone="turret", name="spire"))
        roof_top = top + 0.94
...
def cannon(p: dict, rng) -> Layout:
    ...
    lay.add(Prim("box", (0.12, 0.1, 0.03), (0, top0 + drum_h * 0.55, -(r + 0.035)), slot="banner", seg=1, bone="base", name="banner_patch"))
    _turret_joints(lay, top0, top, muzzle)
...
def crystal_emitter(p: dict, rng) -> Layout:
    ...
    lay.add(Prim("cyl", (0.3, 0.26, 0.08), (0, top0 + ped_h + 0.04, 0), slot="ring", seg=8, bone="turret", name="collar"))
